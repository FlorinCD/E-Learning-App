...
var speedVisiting = 7; // the speed for visiting nodes animation
...
function euclideanDistance(r, c){
    return Math.sqrt(Math.pow((searchpoint_pos[0]-r), 2) + Math.pow((searchpoint_pos[1]-c), 2));
}

function get_heuristics(r, c){
    //console.log("WTF", Math.sqrt(Math.pow((searchpoint_pos[0]-r), 2) + Math.pow((searchpoint_pos[1]-c), 2)));
    return Math.abs(searchpoint_pos[0]-r) + Math.abs(searchpoint_pos[1]-c);
    //return Math.sqrt(Math.pow((searchpoint_pos[0]-r), 2) + Math.pow((searchpoint_pos[1]-c), 2));
}
...
start_obj.addEventListener('click', async function onClick() {
    if(!process_running && !started_process_ended && !process_start && numberStartpoint == 1 && numberSearchpoint == 1){
        ...
        queue.push([get_heuristics(startpoint_pos[0], startpoint_pos[1]), euclideanDistance(startpoint_pos[0], startpoint_pos[1]), startpoint_pos]);

        while(queue.length > 0){
            var element = queue.shift();
            var f = element[0]; // approximated distance
            var euc = element[1]; // euclidean distance for sorting better
            var u = element[2]; // current node
            console.log(u[0], u[1], "Heuristics:", f);
            ...
            if(!visited.has(u[0]+"-"+u[1])){
                ...
                for(var dist_element of my_distance){
                    ...
                    if(0 <= new_r && new_r < n && 0 <= new_c && new_c < m && !visited.has(id) && !walls.has(id)){
                        if(dist.get(u[0]+"-"+u[1]) + cost.get((new_r+"-"+new_c)) < dist.get(id)){
                            dist.set(id, (dist.get(u[0]+"-"+u[1]) + cost.get((new_r+"-"+new_c))));

                            // map with the parent of each node 
                            parent.set(new_r+"-"+new_c, u[0]+"-"+u[1]);
                        }
                        var my_node = [new_r, new_c];
                        var final_h = dist.get(id)+get_heuristics(new_r, new_c); 
                        var euc_dist = euclideanDistance(new_r, new_c);
                        queue.push([final_h, euc_dist, my_node]);
                    }
                }
