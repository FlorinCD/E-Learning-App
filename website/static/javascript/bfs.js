...
document.addEventListener("DOMContentLoaded", function() {
    ...
    clear_obj.addEventListener('click', function onClick() {
        if(!process_running && !process_start){
            ...
            const start_star = document.getElementById("startpoint");
            if(start_star !== null){
                start_star.remove();
            }
            ...
            if(visitedNodes !== null){
              for(const node of visitedNodes){
                ...
                node.style.backgroundColor = color_map["empty"];
                node.style.borderColor = "#abd2ff";
              }
            }

            const shortestPathNodes = document.querySelectorAll(".nodeShortestPath");
    ...
    start_obj.addEventListener('click', async function onClick() {
        if(!process_running && !started_process_ended && !process_start && numberStartpoint == 1 && numberSearchpoint == 1){
            ...
            const queue = [startpoint_pos]; 
            matrix[startpoint_pos[0]][startpoint_pos[1]] = 1;
            var cellfound = false;

            while (queue.length && process_running) {
              if(cellfound) {break;}
                const vertex = queue.shift();
                var r = vertex[0];
                var c = vertex[1];
                let newId = "grid-" + r + "-" + c;
                let u = document.getElementById(newId);
                visitedNodes.push(u);
                ...
                for(let d of dist){
                    if((0 <= parseInt(r)+parseInt(d[0]) && parseInt(r)+parseInt(d[0]) < parseInt(n)) && (parseInt(0) <= parseInt(c)+parseInt(d[1]) && parseInt(c)+parseInt(d[1]) < parseInt(m)) && matrix[parseInt(r)+parseInt(d[0])][parseInt(c)+parseInt(d[1])] == 0){
                        ...
                        parent.set("grid-"+(parseInt(r)+parseInt(d[0]))+"-"+(parseInt(c)+parseInt(d[1])), newId);
                        queue.push([parseInt(r)+parseInt(d[0]),  parseInt(c)+parseInt(d[1])]);
                    }
                ...
                for(const visitedNode of visitedNodes){
                  visitedNodesOrder += 1;
                  setTimeout(() => {
                    visitedNode.className = "visited";
                  }, visitedNodesOrder*speedVisiting);
                }

                if(cellfound){
                  getShortestPath(parent, "grid-"+searchpoint_pos[0]+"-"+searchpoint_pos[1]);
                  for(const node of shortestPath){
                    visitedNodesOrder += 5;
                    setTimeout(() => {
                      const shortestPathNode = document.getElementById(node);
